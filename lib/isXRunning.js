...
async function isLogMgrRunning (regOTHandler, rmOTHandler, timeoutMs = 1500) {
  const { nanoid } = require('nanoid');
  const scopedRedisClient = require('./scopedRedisClient');
  ...
  const retProm = new Promise((resolve) => {
    ...
    regOTHandler('isXRunning:isLogMgrRunningResponse', reqId, async (data) => {
      clearTimeout(timeoutHandle);
      rmOTHandler('isXRunning:isLogMgrRunningResponse', reqId);
      resolve(data);
    });
  });

  // Send health check request directly to the liveLogs health channel
  await scopedRedisClient(async (client, prefix) => client.publish(`${prefix}:liveLogs:health:request`, JSON.stringify({
    action: 'healthCheck',
    requestId: reqId,
    timestamp: Date.now()
  })));
